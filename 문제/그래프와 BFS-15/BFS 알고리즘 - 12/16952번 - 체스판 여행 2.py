...
res = []
visit = defaultdict(int) # 번호/ 좌표 /타입 = dist/ 말바꾼 횟수
visit[(1, sx, sy, 0, 0)] = 0
visit[(1, sx, sy, 1, 0)] = 0
visit[(1, sx, sy, 2, 0)] = 0
temp1 = []
    ...
    while q:
        ...
        if Map[x][y] == num + 1:
            ...
            if num == N*N:
                ...
                continue
            visit[(num, x, y, type, change)] = dist
        # 나이트 이동 0
        if type == 0:
            nd = dist
            nc = change
        ...
        for dx, dy in knight:
            ...
            if 0 <= nx < N and 0 <= ny < N:
                if visit[(num, nx, ny, 0, nc)] == 0 or visit[(num, nx, ny, 0, nc)] > nd + 1:
                    visit[(num, nx, ny, 0, nc)] = nd + 1
                    if nd == dist:
